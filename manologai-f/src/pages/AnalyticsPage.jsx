...
export default function AnalyticsPage() {
    ...
    try {
      ...
      if (lifeRes.ok && lifeJson.entry) {
        ...
        const radarData = [
          ...
          { subject: 'Finances', A: ratings.finances || 0, fullMark: 10 },
          { subject: 'Career', A: ratings.career || 0, fullMark: 10 },
          { subject: 'Partner', A: ratings.partner || 0, fullMark: 10 },
          { subject: 'Family', A: ratings.familyFriends || 0, fullMark: 10 },
          { subject: 'Environment', A: ratings.physicalEnvironment || 0, fullMark: 10 },
          { subject: 'Fun', A: ratings.funRecreation || 0, fullMark: 10 },
          { subject: 'Growth', A: ratings.personalGrowth || 0, fullMark: 10 },
        ];
        setLifeData(radarData);
      }
      ...
      setError("Unable to connect to the intelligence engine. Please check your connection.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const topHabit = useMemo(() => {
    ...
    return habits.reduce((prev, current) => (prev.currentStreak > current.currentStreak) ? prev : current);
  }, [habits]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-950 text-white">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-400 font-mono text-sm tracking-widest animate-pulse uppercase">Analysing life patterns...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen overflow-hidden bg-slate-950 text-slate-100">
      ...
      <div className="relative z-10 flex min-h-screen">
        <motion.aside
          ...
          className="hidden shrink-0 border-r border-white/10 bg-slate-900/40 p-6 backdrop-blur-3xl lg:flex lg:flex-col relative group"
        >
            ...
            >
              <Card className="mt-auto border-white/5 bg-white/5 backdrop-blur-xl shrink-0">
                <CardHeader className="p-4">
                  <CardTitle className="text-[10px] font-mono uppercase tracking-widest text-slate-500">Status</CardTitle>
                  <div className="flex items-center gap-2 mt-2">
                    <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                    <p className="text-xs text-slate-300">Analysis Engine Online</p>
                  </div>
                </CardHeader>
              </Card>
            </motion.div>
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
          <div className="mx-auto max-w-7xl">
            ...
            >
              ...
              <h1 className="text-4xl font-extrabold tracking-tight text-white mb-2">Life Intelligence.</h1>
              <p className="text-slate-400 max-w-2xl text-base">Visualising the correlations between your habits, emotions, and objective daily metrics.</p>
            </motion.header>

            {/* Quick Stats Grid */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }}>
                <Card className="border-white/5 bg-white/[0.02] backdrop-blur-xl group hover:border-pink-500/30 transition-all">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-xs font-mono text-slate-500 uppercase tracking-widest">Emotional State</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-baseline gap-2">
                      <span className="text-3xl font-bold text-white">{avgMood}</span>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
                <Card className="border-white/5 bg-white/[0.02] backdrop-blur-xl group hover:border-emerald-500/30 transition-all">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-xs font-mono text-slate-500 uppercase tracking-widest">Consistency Peak</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {topHabit ? (
                      <>
                        <div className="flex items-baseline gap-2">
                          <span className="text-3xl font-bold text-white">{topHabit.currentStreak}d</span>
                          <span className="text-xs text-slate-500 truncate max-w-[100px]">{topHabit.name}</span>
                        </div>
                      ...
                      <p className="text-slate-500 text-sm italic">No habit data</p>
                    )}
                  </CardContent>
                </Card>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }}>
                <Card className="border-white/5 bg-white/[0.02] backdrop-blur-xl group hover:border-indigo-500/30 transition-all">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-xs font-mono text-slate-500 uppercase tracking-widest">Life Score</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-baseline gap-2">
                      <span className="text-3xl font-bold text-white">
                        {lifeData.length > 0 ? (lifeData.reduce((acc, c) => acc + c.A, 0) / 8).toFixed(1) : "—"}
                      </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                <Card className="border-white/10 bg-slate-900/50 backdrop-blur-xl overflow-hidden min-h-[420px]">
                  <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2">
                      <Activity className="h-5 w-5 text-pink-500" />
                      Mood Dynamics
                    </CardTitle>
                    <CardDescription>Fluctuations in your reported emotional state over time.</CardDescription>
                  </CardHeader>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                <Card className="border-white/10 bg-slate-900/50 backdrop-blur-xl overflow-hidden min-h-[420px]">
                  <CardHeader>
                    ...
                    <CardDescription>Visualisation of your current life domain ratings.</CardDescription>
                  </CardHeader>
                  <CardContent className="h-[300px] w-full">
                    {lifeData.length > 0 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={lifeData}>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="lg:col-span-2">
                <Card className="border-white/10 bg-slate-900/50 backdrop-blur-xl overflow-hidden min-h-[420px]">
                  <CardHeader>
                    ...
                    <CardDescription>Comparison of Sleep, Screen, and Work/Study time over the last week.</CardDescription>
                  </CardHeader>
                  <CardContent className="h-[300px] w-full">
                    {timeData.length > 0 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={timeData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                          ...
                          <Bar dataKey="Sleep" stackId="a" fill="#10b981" radius={[0, 0, 0, 0]} />
                          <Bar dataKey="Work" stackId="a" fill="#3b82f6" radius={[0, 0, 0, 0]} />
                          <Bar dataKey="Screen" stackId="a" fill="#ef4444" radius={[4, 4, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    ) : (
                      <div className="h-full flex items-center justify-center text-slate-600 text-sm font-mono italic">
